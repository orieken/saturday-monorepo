...
// newGenerateServiceTool wires the tool with the real service generator.
// Same shape as generate_site's test helper.
func newGenerateServiceTool(t *testing.T) *GenerateServiceTool {
	t.Helper()
	gen := generators.NewServiceGenerator(newProcessor(t), newValidator())
	return NewGenerateServiceTool(silentLogger(), gen)
}

func TestGenerateServiceTool_Metadata(t *testing.T) {
	tool := newGenerateServiceTool(t)

	if tool.Name() != "generate_service" {
		t.Errorf("Name: got %q, want %q", tool.Name(), "generate_service")
	}
	if tool.Description() == "" {
	...
	wantRequired := map[string]bool{"name": true, "baseUrl": true, "endpoints": true}
...
func TestGenerateServiceTool_Execute_Success_NoWrite(t *testing.T) {
	tool := newGenerateServiceTool(t)
	...
	req := buildRequest(map[string]any{
		...
		"endpoints": []any{
			map[string]any{"name": "getUser", "method": "GET", "path": "/users/1"},
			map[string]any{"name": "createUser", "method": "POST", "path": "/users"},
		},
	})
	...
	if !strings.Contains(got.Code, "UserService") {
		t.Errorf("expected code to contain UserService, got:\n%s", got.Code)
	}
	if !strings.Contains(got.Code, "extends BaseService") {
		t.Errorf("expected code to extend BaseService, got:\n%s", got.Code)
	}
	if got.FileName != "user-service.ts" {
		t.Errorf("FileName: got %q, want %q", got.FileName, "user-service.ts")
	}
	if got.Written {
...
func TestGenerateServiceTool_Execute_Success_WithWrite(t *testing.T) {
	tool := newGenerateServiceTool(t)
	outDir := t.TempDir()
	...
	req := buildRequest(map[string]any{
		...
		"baseUrl": "https://api.example.com",
		"endpoints": []any{
			map[string]any{"name": "listOrders", "method": "GET", "path": "/orders"},
		},
		"writeToFile": true,
		"outputPath":  outDir,
	})
	...
	expectedPath := filepath.Join(outDir, "lib", "services", got.FileName)
...
func TestGenerateServiceTool_Execute_WriteWithoutOutputPathErrors(t *testing.T) {
	tool := newGenerateServiceTool(t)
...
func TestGenerateServiceTool_Execute_WriteFailureSurfacesAsToolError(t *testing.T) {
	tool := newGenerateServiceTool(t)
	outDir := t.TempDir()
...
func TestGenerateServiceTool_Execute_InvalidRequestSurfacesAsToolError(t *testing.T) {
	tool := newGenerateServiceTool(t)
